...
dados_rotas_t *nova_rota(dados_aeronaves_t *lista_aeronaves) {
    ...
    to_upper_string(rota->local_partida); // Novo: Maiúsculas

    printf("Local de destino................: ");
    fgets(rota->local_destino, STR_SIZE, stdin);
    remover_enter(rota->local_destino);
    to_upper_string(rota->local_destino); // Novo: Maiúsculas
    ...
    if (aero_ref->tipo == PASSAGEIRO) {
        ...
        do {
            printf("Quantidade de carga kg (Max %d): ", aero_ref->qtd_carga_util);
    ...
    for(int i = 0; i < rota->num_membros; i++) {
        ...
        to_upper_string(rota->nomes_membros[i]); // Novo: Maiúsculas
    }

    rota->prox = NULL;
    return rota;
}
// ... (resto das funções de busca e inserção permanecem iguais)
...
void alterar_status_aeronave(dados_aeronaves_t **lista_aeronaves) {
    ...
    if((status_t)novo_status == MANUTENCAO && aeronave->status != MANUTENCAO) {
        ...
        printf("Aeronave entrou em manutenção pela %dª vez.\n", aeronave->qtd_manutencoes);
    }

    aeronave->status = (status_t)novo_status;
    printf("Status da aeronave ID: %d alterado para %s.\n", id_escolhido, 
